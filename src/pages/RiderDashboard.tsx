...
export default function RiderDashboard() {
  ...
  const updateLocationInDb = (lat: number, lng: number) => {
    ...
    const currentLocs = db.getRiderLocations();
    const filtered = currentLocs.filter(l => {
      if (l.riderId === user.id) return false;
      const resolvedL = db.resolveUser(l.riderId);
      return !resolvedL || resolvedL.email.toLowerCase() !== user.email.toLowerCase();
    });

    db.saveRiderLocations([...filtered, newLoc]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-12">
                ...
                <div key={s.id} className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">{est?.address || 'Sem endereço cadastrado'}</p>
                    </div>
      ...
      {activeScheduleChatId && (
        <ScheduleChatModal 
          scheduleId={activeScheduleChatId}           onClose={() => setActiveScheduleChatId(null)} 
        />
      )}
    </div>
  );
