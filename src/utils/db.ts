    ...
    try {
      ...
      if (!schsError && schs) {
        const mappedSchs: Schedule[] = schs
          ...
          .map(s => {
            ...
            return {
              id: s.id,
              riderId: userIdMap.get(s.rider_id) || s.rider_id, // MAPEAR ID DO MOTOBOY PARA UUID
              establishmentId: estIdMap.get(s.establishment_id) || s.establishment_id, // MAPEAR ID DO ESTABELECIMENTO PARA UUID
              date: s.date,
      ...
      if (!delsError && dels) {
        const mappedDels: Delivery[] = dels
          ...
          .map(d => {
            ...
            return {
              id: d.id,
              riderId: userIdMap.get(d.rider_id) || d.rider_id, // MAPEAR ID DO MOTOBOY PARA UUID
              establishmentId: estIdMap.get(d.establishment_id) || d.establishment_id, // MAPEAR ID DO ESTABELECIMENTO PARA UUID
              date: d.date,
              time: d.time,
      ...
      if (!notifsError && notifs) {
        const mappedNotifs: Notification[] = notifs
          ...
          .map(n => ({
            id: n.id,
            riderId: userIdMap.get(n.rider_id) || n.rider_id, // MAPEAR ID DO MOTOBOY PARA UUID
            title: n.title,
            message: n.message,
            date: n.date,
            read: n.read
...
<dyad-chat-summary>Corrigido o sumiço de corridas mapeando IDs antigos para UUIDs em tempo real na sincronização</dyad-chat-summary>

Prontinho! Corrigi o problema de sincronização mapeando os IDs antigos para UUIDs em tempo real durante o processo de `pullFromSupabase`. Agora as corridas de Rodrigo e Pedro Henrique permanecerão visíveis e salvas de forma consistente mesmo após atualizar a página!<dyad-write path="src/utils/db.ts" description="Garantindo a integridade completa do arquivo db.ts com todos os resolvedores e mapeamentos de IDs">
"use client";
