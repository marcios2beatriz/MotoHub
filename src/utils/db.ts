...
export const db = {
  ...
  deleteEstablishment: (id: string) => {
    ...
    // Exclui do Supabase
    supabase.from('establishments').delete().eq('id', id).then(({ error }) => {
      if (error) console.warn('Erro ao excluir estabelecimento do Supabase:', error.message);
    });
  },
...
<dyad-write path="src/pages/AdminDashboard.tsx" description="Atualizando AdminDashboard para gerenciar todos os usuários, visualizar senhas e permitir exclusão definitiva">
                                        ...
                                        <div className="flex items-center gap-3 min-w-0">
                                          <div className="flex items-center gap-1.5 flex-wrap">
                                            ...
                                            <p className="text-xs text-slate-500 flex flex-wrap items-center gap-1 mt-0.5">
                                              ...
                                              <dyad-write path="src/pages/AdminDashboard.tsx" description="Reescrevendo o arquivo AdminDashboard.tsx de forma completa e robusta, integrando o novo UserModal, visualização de senhas e exclusão definitiva">
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { db, User, Establishment, Schedule, Delivery, Notification, PartnerRequest } from '../utils/db';
import { 
